...
import {CUENTAS_SEL, pdcNm} from './core.js';
...
export function buscarCuentas(q,limite=40){
  const t=norm(q).trim();
  if(!t)return CUENTAS_SEL.slice(0,limite);
  const palabras=t.split(/\s+/);
  const scored=[];
  for(const c of CUENTAS_SEL){
...
export function inputCuenta({id,value='',onPick='',placeholder='Buscar cuenta…',clase='linea-inp'}){
